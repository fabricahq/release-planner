...
func cfg(t *testing.T, version string) config.Config {
	t.Helper()
	c, err := config.Parse([]byte("version: " + version + "\nfirst-version: v1.0.0\nvalidate:\n  go: '1.27.x'\n  run: |\n    go install example.com/tool@v1\n    tool check\n"))
	if err != nil {
		t.Fatal(err)
	}
	return c
...
func TestInstallIsIdempotent(t *testing.T) {
	...
	got := actions(changes)
	for _, p := range []string{WorkflowPath, AgentsSkillPath, ClaudeSkillPath, "releases/README.md"} {
		if got[p] != "created" {
...
func TestCheckFindsDrift(t *testing.T) {
	...
	problemFor(t, Check(root, cfg(t, "v0.3.0")), WorkflowPath, "generated by v0.2.0, but release-planner.yml pins v0.3.0")

	changed := cfg(t, "v0.2.0")
	changed.Validate.Run = "make test"
	problemFor(t, Check(root, changed), WorkflowPath, "differs from what release-planner.yml generates")
...
func TestPolicyIsSeededOnce(t *testing.T) {
	...
	if !strings.Contains(read(t, root, "releases/README.md"), "The first release is v1.0.0.") {
		t.Fatal("policy starter")
	}
	put(t, root, "releases/README.md", "Our policy.\n")
	if _, err := Install(root, c, false); err != nil {
		t.Fatal(err)
	}
	if read(t, root, "releases/README.md") != "Our policy.\n" {
...
func TestUninstall(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "releases/README.md")); err != nil {

...
Whatever the style says, every release keeps the parts `draft` writes: the **Pull Requests** section listing every pull request, grouped and formatted as the style says, and the **Full Changelog** link (or, for a first release, a link to the tagged source).

...
When your agent checks its notes, a broken rule stops it until it fixes the notes. On the release pull request, a broken rule is only a warning in the status comment, so you can break one on purpose. To stop checking a rule, turn it off in `.release-planner/config.yml`:

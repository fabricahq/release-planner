...
| `report --needs <json> --branch <name> (--pull-request <n> \| --merged <sha>) [--plan <file>] [--assets <dir>] [--downstream <owner/name:workflow.yml>...]` | CI | Writes the release status section of the release pull request's description, from the workflow's `needs` context and the plan. When a downstream job failed, reads each target's job result from the run. Warns instead of failing when it can't comment. |
...
| **report** | Always, after the others | `contents: read`, `actions: read`, `pull-requests: write` | Writes the release status section of the pull request description with `report`, and comments on a failure after the merge. |
...
The report job keeps a section at the end of the release pull request's description, between the hidden markers `<!-- release-planner:status:start -->` and `<!-- release-planner:status:end -->`, and rewrites only the text between them, reading the description just before each edit; it skips the edit when nothing changed. The section opens with a `### Release status` heading and a table of the version, the release commit, and the previous release. On the pull request it shows broken release notes rules, the jobs' results, settings warnings, and the assets with their sizes and a link to download them; for a notes edit, which releases merging updates, with a warning if their notes on GitHub were edited since they merged. After the merge, the same section shows the published release or updated notes, with a download link for each asset, the downstream workflows, or the failed job with a link to the run and the retry. Because GitHub doesn't notify anyone mentioned in an edited description, a failure after the merge also gets a short comment that mentions whoever merged, names the failed job, and links the run; a hidden `<!-- release-planner:failure run=... attempt=... job=... -->` marker keeps a re-run of the report from posting it twice. The report never adds a section to a pull request that doesn't request a release or edit notes, and posts no other comments. When the token can't edit the pull request, as on a fork's pull request, it warns and writes the report to the step summary instead.

## Retrying a failed release

Read the release status in the release pull request's description, the failed run, and any existing tag or release before acting. Prefer **Re-run failed jobs** on the failed run: it reuses the same plan and files. For a manual retry, run the workflow on the release branch with `merged-commit` set to the full SHA of the commit the release pull request merged as; never another commit. It plans the same release commit, and reuses the pull request's run when it can; otherwise it builds the assets again, and publish refuses a draft that already holds different files. A retry that finds the release already published checks its tag and files, and keeps its notes even if a later pull request edited them.

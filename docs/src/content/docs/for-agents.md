...
3. **release-assets** (optional): calls the repository's [build workflow](/customize/release-assets/) with the planned commit, tag, and version (the tag without its `v`), read access, and no secrets. It uploads the files to attach as the `release-assets` artifact. It runs only on the release branch, never for a pull request.
4. **attest** (with release-assets): downloads the artifact and signs a build provenance attestation for every file. It checks out no code and can't write to the repository.
5. **publish**: the only job with permission to write. It runs Release Planner at the pinned version and none of the repository's code. Before writing, it confirms that the version tags haven't changed since validation, that the previous release is published, and that any existing tag or release matches the plan. It then creates the tag and release, and afterward verifies that the tag points to the approved commit. With release assets, it passes the artifact to `publish --assets`, which refuses an empty artifact or anything but plainly named files, stages them on a draft release, verifies GitHub's checksums for them, and publishes last. If a matching release is already published, it changes nothing.

The attest and publish jobs use the `release` environment, and publish runs one publication at a time across the repository.

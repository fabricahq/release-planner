...
| `report --needs <json> --branch <name> (--pull-request <n> \| --merged <sha>) [--plan <file>] [--assets <dir>] [--downstream <owner/name:workflow.yml>...]` | CI | Writes the release status comment on the release pull request, from the workflow's `needs` context and the plan. When a downstream job failed, reads each target's job result from the run. Warns instead of failing when it can't comment. |

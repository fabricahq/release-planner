...
| **attest-release** | With assets; after a new release publishes | `contents: read`, `id-token: write`, `attestations: write` | Downloads the published assets with `gh release download` and attests them again, from the release branch, so consumers can require an attestation made on it. Runs no repository code. |
| **downstream** | Optional; after a new stable release publishes, and with assets, after attest-release | `contents: read`, plus a GitHub App token for the targets | In the `downstream` environment. One job per target, named `downstream (<owner/name>:<workflow>)`, starts that [downstream workflow](/customize/downstream/). |

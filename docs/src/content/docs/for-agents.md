...
| `report --needs <json> --branch <name> (--pull-request <n> [--head-ref <branch>] [--head-repository <owner/name>] \| --merged <sha>) [--plan <file>] [--assets <dir>] [--downstream <owner/name:workflow.yml>...]` | CI | Writes the release status blocks of the release pull request's description, from the workflow's `needs` context, the plan, and the run's jobs, which it links. `--head-ref` and `--head-repository` name the pull request's branch, for the link that edits the notes. Warns instead of failing when it can't read the jobs, edit the description, or comment. |
